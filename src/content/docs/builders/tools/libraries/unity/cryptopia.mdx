...
<ToolLayout
    introParagraph="The `Cryptopia.SkaleGasMiner` is an optimized solution for mining sFUEL (Skale gas) in Unity3D. This library provides an efficient and minimal API for managing gas mining processes tailored for the Skale network."
